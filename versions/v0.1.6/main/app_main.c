...
{
#if CONFIG_PANEL_SD_CARD
    esp_err_t ret = bsp_sdcard_mount();
    if (ret == ESP_OK) {
        s_state.sd_mounted = true;
        snprintf(s_state.last_log, sizeof(s_state.last_log), "SD montada: /sdcard");
        ESP_LOGI(TAG, "microSD mounted");
    } else {
        s_state.sd_mounted = false;
        snprintf(s_state.last_log, sizeof(s_state.last_log), "SD no montada: %s", esp_err_to_name(ret));
        ESP_LOGW(TAG, "microSD not mounted: %s", esp_err_to_name(ret));
    }
#else
    s_state.sd_mounted = false;
    snprintf(s_state.last_log, sizeof(s_state.last_log), "SD desactivada");
#endif
}
