...
#define PANEL_UI_VERSION "v0.1.6"
...
typedef struct {
    ...
    lv_obj_t *settings_ota_bar;
    lv_obj_t *settings_sd;
    lv_obj_t *settings_last_log;
...
static void create_header(lv_obj_t *root)
{
    ...
    s_ui.wifi_label = label(root, "WiFi --", &lv_font_montserrat_16, UI_COLOR_MUTED);
    lv_obj_set_pos(s_ui.wifi_label, 806, 20);

    s_ui.version_label = label(root, PANEL_UI_VERSION, &lv_font_montserrat_16, UI_COLOR_MUTED);
    lv_obj_set_pos(s_ui.version_label, 882, 20);

    lv_obj_t *settings = button(root, 940, 9, 64, 40, "SET", false);
...
static void create_vehicle_page(lv_obj_t *parent)
{
    ...
    s_ui.vehicle_status = label(hero, "Cable -- | Puertas --", &lv_font_montserrat_14, UI_COLOR_TEXT);
    lv_obj_set_pos(s_ui.vehicle_status, 292, 160);
...
static void create_settings_page(lv_obj_t *parent)
{
    ...
    lv_obj_t *net = card(p, 32, 72, 448, 190);
    ...
    s_ui.settings_sd = label(net, "SD: --", &lv_font_montserrat_16, UI_COLOR_TEXT);
    lv_obj_set_pos(s_ui.settings_sd, 0, 124);
    lv_obj_t *ssid = label(net, cfg->wifi_ssid[0] ? cfg->wifi_ssid : "SSID no configurado", &lv_font_montserrat_16, UI_COLOR_MUTED);
    lv_obj_set_pos(ssid, 0, 152);
    ...
    lv_obj_t *ota_btn = button(svc, 310, 136, 130, 46, "OTA", true);
    lv_obj_add_event_cb(ota_btn, ota_event, LV_EVENT_CLICKED, NULL);
    s_ui.settings_ota_bar = lv_bar_create(svc);
    lv_obj_set_pos(s_ui.settings_ota_bar, 0, 192);
    lv_obj_set_size(s_ui.settings_ota_bar, 440, 12);
    lv_bar_set_range(s_ui.settings_ota_bar, 0, 100);
    lv_bar_set_value(s_ui.settings_ota_bar, 0, LV_ANIM_OFF);
    lv_obj_set_style_bg_color(s_ui.settings_ota_bar, lv_color_hex(UI_COLOR_LINE), LV_PART_MAIN);
    lv_obj_set_style_bg_color(s_ui.settings_ota_bar, lv_color_hex(UI_COLOR_PRIMARY), LV_PART_INDICATOR);
    ...
    s_ui.settings_last_log = label(urls, "Log: --", &lv_font_montserrat_16, UI_COLOR_MUTED);
    lv_obj_set_pos(s_ui.settings_last_log, 0, 132);
}
...
void dashboard_ui_set_state(const dashboard_state_t *state)
{
    ...
    if (s_ui.vehicle_battery) {
        ...
        snprintf(text, sizeof(text), "Cable %s | Puertas %s", state->vehicle.cable_connected ? "conectado" : "desconectado", state->vehicle.doors_locked ? "OK" : "abiertas");
        lv_label_set_text(s_ui.vehicle_status, text);
    }
    ...
    if (s_ui.ota_panel) {
        ...
        if (ota_state == OTA_SERVICE_IDLE) {
            ...
            lv_obj_remove_flag(s_ui.ota_panel, LV_OBJ_FLAG_HIDDEN);
            const char *ota_message = ota_service_get_message();
            if (strcmp(lv_label_get_text(s_ui.ota_label), ota_message) != 0) {
                lv_label_set_text(s_ui.ota_label, ota_message);
            }
            if (lv_bar_get_value(s_ui.ota_bar) != ota_progress) {
                lv_bar_set_value(s_ui.ota_bar, ota_progress, LV_ANIM_OFF);
            }
        }
    }

    if (s_ui.wifi_label) {
        snprintf(text, sizeof(text), "WiFi %s", state->wifi_connected ? "OK" : "--");
        lv_label_set_text(s_ui.wifi_label, text);
        lv_obj_set_style_text_color(s_ui.wifi_label, lv_color_hex(state->wifi_connected ? UI_COLOR_PRIMARY : UI_COLOR_MUTED), 0);
    }
    ...
    if (s_ui.settings_wifi) {
        ...
        snprintf(text, sizeof(text), "SD: %s", state->sd_mounted ? "montada" : "no montada");
        lv_label_set_text(s_ui.settings_sd, text);
        snprintf(text, sizeof(text), "Log: %.80s", state->last_log);
        lv_label_set_text(s_ui.settings_last_log, text);
        if (s_ui.settings_ota_bar && lv_bar_get_value(s_ui.settings_ota_bar) != ota_service_get_progress_percent()) {
            lv_bar_set_value(s_ui.settings_ota_bar, ota_service_get_progress_percent(), LV_ANIM_OFF);
        }
    }
}
