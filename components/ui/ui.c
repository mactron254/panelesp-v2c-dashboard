...
static void create_header(lv_obj_t *root)
{
    ...
    label(root, "WiFi", &lv_font_montserrat_16, UI_COLOR_TEXT);
    lv_obj_t *wifi = lv_obj_get_child(root, lv_obj_get_child_count(root) - 1);
    lv_obj_set_pos(wifi, 836, 20);

    label(root, "Hora", &lv_font_montserrat_16, UI_COLOR_TEXT);
    lv_obj_t *clock = lv_obj_get_child(root, lv_obj_get_child_count(root) - 1);
    lv_obj_set_pos(clock, 890, 20);
...
static void create_nav(lv_obj_t *root)
{
    ...
    const char *names[] = {"Cargador", "EVCC", "Solar", "Vehiculo"};
    int enabled_count = 0;
    for (int i = 0; i < 4; ++i) {
        enabled_count += screen_enabled((dashboard_screen_t)i) ? 1 : 0;
    }
    if (enabled_count == 0) {
        enabled_count = 1;
    }
...
void dashboard_ui_init(dashboard_state_t *state)
{
    ...
    s_ui.status_label = label(s_ui.root, "Modo demo", &lv_font_montserrat_14, UI_COLOR_MUTED);
    lv_obj_set_pos(s_ui.status_label, 20, 548);
...
void dashboard_ui_set_state(const dashboard_state_t *state)
{
    ...
    dashboard_ui_show_status(state->status_message);
}
